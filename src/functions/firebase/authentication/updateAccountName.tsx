...
export function firebaseUpdateAccountName(user: User, newAccountName: string) {
	if (user) {
		...
		return notify.promise(
			updateProfile(user, { displayName: newAccountName }),
			{
				loading: "Updating account name...",
				success: "Account name updated successfully.",
				error: (err) => `Error updating account name: ${(err as ErrorType).message} ${(err as ErrorType).code}`,
			}
		);
	} else {
